...
namespace game00
{
    public partial class Start1 : Form
    {
        ...
        private int[] imageIndices;             // 이미지 인덱스 배열
        private Random random = new Random();   // 랜덤 객체
        private int[] index;                    // 챔피언 인덱스 배열
        private Button selectedChampionButton;   // 선택된 챔피언 버튼
        private Button selectedFieldButton;      // 선택된 필드 버튼
        ...
        private Timer countdownTimer;           // 타이머 객체
        private int remainingTime = 15;        // 남은 시간
        ...
        private void CountdownTimer_Tick(object sender, EventArgs e)
        {
            ...
            progressBar1.Value = remainingTime; // 진행 바 업데이트

            // 시간이 다 되면
            if (remainingTime <= 0)
            {
                countdownTimer.Stop();           // 타이머 중지
                MessageBox.Show("시간이 다 되었습니다! 진행 상황을 저장합니다..."); // 알림
                //레벨포인트 증가
        ...
        public List<Image> championImages1 = new List<Image>();
        public void fieldscan()
        {
            ...
            for (int i = 1; i <= 21; i++)
            {
                ...
                if (fieldButton != null)
                {
                    championImages1.Add(fieldButton.BackgroundImage); // 이미지가 null일 경우도 포함
                }
            }

            // 21개의 이미지를 맞춰서 null로 패딩
            while (championImages1.Count < 21)
            {
                championImages1.Add(null); // null 추가하여 21개 맞춤
            }
        }
        ...
        {
            fieldscan();
            Start2 form2 = Start2.GetInstance(this, championImages1);
            form2.Show();  // Form2 표시
            this.Hide();   // 현재 폼 숨기기
        }

        // 랜덤 챔피언 설정 메서드
        public void randomchampion()
        {
        ...
        // 버튼 클릭 이벤트 핸들러
        private void btn_select1_Click(object sender, EventArgs e) { if (DeductMoney(index[0])) PlaceChampion(btn_select1); }
        private void btn_select2_Click(object sender, EventArgs e) { if (DeductMoney(index[1])) PlaceChampion(btn_select2); }
        private void btn_select3_Click(object sender, EventArgs e) { if (DeductMoney(index[2])) PlaceChampion(btn_select3); }
        private void btn_select4_Click(object sender, EventArgs e) { if (DeductMoney(index[3])) PlaceChampion(btn_select4); }
        private void btn_select5_Click(object sender, EventArgs e) { if (DeductMoney(index[4])) PlaceChampion(btn_select5); }
        ...
        private void levelupup()
        {
            ...
            if (currentLevelPoint > nextLevelPoint)
            {
                lb_levelpoint.Text = "0"; // 포인트 초기화
                lb_level.Text = Convert.ToString(currentLevel + 1); // 레벨 증가
                UpdateLevelPointRequirement(currentLevel + 1); // 레벨 요구량 업데이트
        ...
        private void UpdateLevelPointRequirement(int level)
        {
            ...
            {
                case 4:
                    lb_1.Text = "/20"; // 다음 레벨 포인트 요구량 설정
                    break;
                case 5:
                    lb_1.Text = "/30"; // 다음 레벨 포인트 요구량 설정
                    break;
                case 6:
                    btn_levelup.Enabled = false; // 레벨업 버튼 비활성화
                    break;
            }
        }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= 9; i++)
            {
                ...
                {
                    championButton.Click += ChampionButton_Click; // 이벤트 핸들러 추가
                }
            }
            ...
            for (int i = 1; i <= 21; i++)
            {
                ...
                if (fieldButton != null)
                {
                    fieldButton.Click += FieldButton_Click; // 이벤트 핸들러 추가
                }
            }
        }
        public void UpdateTeam1Images(List<Image> images)
        {
            for (int i = 0; i < 21; i++)
            {
